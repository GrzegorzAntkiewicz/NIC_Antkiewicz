...
#Read in Birth rate data (crude per 1000 population) from worldbank
...
corr.GDP.mortality <- cor(final.df$death_under_5_per_birth,
...
plot(final.df$GDP_PPP_2010,
     ...
     ylab  = "Death under 5 per birth (in percent)")

#Correlation gini coefficient and mortality

corr.gini.mortality <- cor(final.df$death_under_5_per_birth,
                          final.df$gini_disp, 
                          method ="pearson")
